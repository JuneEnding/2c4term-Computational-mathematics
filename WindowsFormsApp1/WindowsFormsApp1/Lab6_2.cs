...
namespace LabWorkCompMath.Lab2{
    class Lab6_2 : Lab
    {
        ...
        //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        public Lab6_2() : this(null) { }

        public Lab6_2(Button relatedButton) : base(relatedButton)
        {
        ...
        float tem;
        float res;
        public float SimpsonIntegral(float[] y, float[] bord, float step) {
            tem = new float();
            res = new float();
            if(y.Length%2==1)
                Array.Resize(ref y, y.Length-1);
            res = y[0] + y[y.Length - 1];
            tem = 0;
        ...
        public float Inaccuracy(float[] y, float[] bord) {
            return Max(y)* (float)Math.Pow((bord[0] + bord[1]) / y.Length,4);
        }
